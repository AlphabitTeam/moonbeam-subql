...
export async function createDelegation(
  data: DispatchedEventData
): Promise<void> {
  const [delegatorId, value, candidateId, added] =
    data.rawEvent.event.data.toJSON() as [string, string, string, string];
 ...
 * @param data
 */
export async function changeDelegation(
  data: DispatchedEventData
): Promise<void> {
  const [delegatorId, candidateId, value, isTop] =
    data.rawEvent.event.data.toJSON() as [string, string, string, boolean];
  const delegator = await ensureDelegator(delegatorId);
  const candidate = await ensureCandidate(candidateId);
  const entity = await ensureDelegation(delegator.id, candidate.id);
  entity.value = BigInt(value);
...
export async function removeDelegator(
  ...
  await removeAllDelegations(
    data.rawEvent.event.data[0].toString().toLowerCase()
  );
}
